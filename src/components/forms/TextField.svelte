...
<label for="{id}" class="text-field{hasError?' error':''}{success?' success':''}">
    <div class="text-field_wrapper">
        ...
        <input class="text-field_input" type="text" id="{id}" name="{name}" placeholder="{placeholder}" required="{required}" autocomplete="{autocomplete}" on:focus={onFocus} on:blur={onBlur}  bind:value={value} pattern={pattern}/>
    </div>
